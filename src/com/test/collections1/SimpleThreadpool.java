...
public class SimpleThreadpool {
	...
	public static void main(String[] args) {
		SimpleThreadpool exe = getInstance();
		for (int i = 0; i < 10; i++) {  
	...
	static class MyThread implements Runnable {
		...
		@Override
		public void run() {
			System.out.println("am in run method.." + Thread.currentThread().getName());
		}
	...
	private SimpleThreadpool(int threadCount) {
		...
		for (int threadIndex = 0; threadIndex < threadCount; threadIndex++) {
			SimpleThreadpoolThread thread = new SimpleThreadpoolThread(
					"SimpleThreadpool" + poolCount.get() + "Thread" + threadIndex, this.execute, this.runnables);
			thread.start();
	...
	/**
	 * Gets a new threadpool instance with number of threads equal to the number
	 * of processors (or CPU threads) available
	 *
